        ...
        <div className="border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 rounded-xl p-4 font-mono shadow-sm overflow-hidden relative group">
            ...
            <div className="mt-4 pt-3 border-t border-zinc-100 dark:border-zinc-800">
                <div className="flex items-center justify-between mb-2">
                     <span className="text-[10px] font-bold uppercase text-zinc-500">Live Telemetry</span>
                     {isLoading ? (
                        <div className="w-3 h-3 border-2 border-indigo-500 border-t-transparent rounded-full animate-spin"></div>
                     ) : (
                        <button onClick={fetchTelemetry} className="text-[9px] text-indigo-500 hover:underline">REFRESH</button>
                     )}
                </div>
                ...
                {telemetry ? (
                    ...
                    <div className="text-[10px] text-zinc-500 italic py-2">Signal disconnected.</div>
                )}
            </div>
        </div>
    );
