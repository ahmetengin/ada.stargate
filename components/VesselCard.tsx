...
import { Activity, ShieldCheck, User, Hash, Zap, Droplets, Gauge, Thermometer, Anchor } from 'lucide-react';
...
export const VesselCard: React.FC<VesselCardProps> = ({ profile }) => {
    ...
    return (
        <div className="mt-6 border border-white/5 bg-[#0a121e] p-4 rounded-xl font-mono text-xs shadow-xl animate-in fade-in slide-in-from-left-2 max-w-md relative overflow-hidden group">
            ...
            <div className="grid grid-cols-2 gap-y-3 gap-x-4">
                ...
                <div className="flex flex-col">
                    ...
                    <span className="text-zinc-300 truncate">{profile.type}</span>
                </div>
            ...
            <div className="mt-4 pt-3 border-t border-white/5">
                <button 
                    ...
                    className={`w-full flex items-center justify-center gap-2 py-2 rounded text-[10px] font-bold uppercase tracking-wider transition-all ${
                        ...
                        : 'bg-teal-500/10 text-teal-400 border border-teal-500/30 hover:bg-teal-500/20 hover:border-teal-500/50'
                    }`}
                >
                    {isLoading ? (
            ...
            {/* Telemetry Panel */}
            {telemetry && (
                <div className="mt-3 bg-black/40 rounded-lg p-3 border border-white/5 animate-in slide-in-from-top-2 space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                        ...
                        <div className="bg-zinc-900/50 p-2 rounded border border-white/5">
                            ...
                            <div className="text-sm font-bold text-white">{telemetry.battery.serviceBank}V</div>
                        ...
                        <div className="bg-zinc-900/50 p-2 rounded border border-white/5">
                            ...
                            <div className={`text-sm font-bold ${telemetry.shorePower.connected ? 'text-emerald-400' : 'text-zinc-500'}`}>
                        ...
                        {/* Tanks */}
                        <div className="col-span-2 bg-zinc-900/50 p-2 rounded border border-white/5 space-y-2">
                            <div className="flex items-center justify-between text-[9px]">
                                <span className="text-zinc-500 flex items-center gap-1"><Droplets size={10}/> Fuel</span>
                                <span className="text-amber-500 font-bold">{telemetry.tanks.fuel}%</span>
                            </div>
                            ...
                            <div className="flex items-center justify-between text-[9px]">
                                ...
                                <span className="text-blue-500 font-bold">{telemetry.tanks.freshWater}%</span>
                            </div>
                            <div className="w-full bg-zinc-800 h-1 rounded-full overflow-hidden">
                        ...
                        {/* Climate (Ada Sea ONE) */}
                        {telemetry.comfort && (
                            <div className="col-span-2 flex justify-between items-center bg-indigo-900/20 p-2 rounded border border-indigo-500/20">
                                <div className="flex items-center gap-2">
                                    <Thermometer size={12} className="text-indigo-400" />
                                    <div>
                                        <div className="text-[9px] text-indigo-300 font-bold">HVAC / {telemetry.comfort.climate.zone}</div>
                                        <div className="text-[9px] text-indigo-400/70">{telemetry.comfort.climate.mode}</div>
                                    </div>
                                </div>
                                <div className="text-sm font-bold text-white">{telemetry.comfort.climate.currentTemp}°C</div>
                            </div>
                        )}
                    </div>
                </div>
