...
import { Radio, Video, Crown, CalendarCheck, X, AlertTriangle } from 'lucide-react';
import { RegistryEntry, AisTarget } from '../../../types';
...
export const LiveMap: React.FC<LiveMapProps> = ({ registry = [], aisTargets = [] }) => {
  ...
  // Map Physical Berths to Dynamic Data
  // In a real app, this would use a GeoJSON coordinate mapper.
  // Here we simulate the mapping of registry data to SVG coordinates.
  const berths = [
    { id: 'A-N1', cx: 60, cy: 35, orientation: 'up' },
    { id: 'A-N2', cx: 80, cy: 35, orientation: 'up' },
    { id: 'A-N3', cx: 100, cy: 35, orientation: 'up' },
    { id: 'A-S1', cx: 60, cy: 43, orientation: 'down' },
    { id: 'A-S2', cx: 80, cy: 43, orientation: 'down' },
    { id: 'A-S3', cx: 100, cy: 43, orientation: 'down' },
    { id: 'VIP-01', cx: 230, cy: 50, orientation: 'left' }
  ].map((b, i) => {
      // Hydrate with Registry Data if available, else Mock or Empty
      const vessel = registry[i] || null;
      return {
          ...b,
          status: vessel ? 'OCCUPIED' : 'EMPTY',
          vessel: vessel ? vessel.vesselName : null,
          type: vessel ? 'Yacht' : null,
          relationship: vessel ? (i % 2 === 0 ? 'CONTRACT_HOLDER' : 'VISITOR') : null
      };
  });
    ...
    <div className="w-full h-full relative bg-slate-200 dark:bg-[#050b14] flex items-center justify-center overflow-hidden transition-colors bg-grid-pattern">
        ...
        <svg viewBox="0 0 300 150" className="w-full h-full z-10" onClick={() => setSelectedVessel(null)}>
            ...
            {berths.map(b => {
                ...
                let fillClass = "fill-slate-600 dark:fill-[#52525b]"; 
                if (selectedVessel && selectedVessel.id === b.id) fillClass = "fill-indigo-500 dark:fill-[#6366f1]";

                let ringColor = "";
                if (b.relationship === 'CONTRACT_HOLDER') ringColor = "#fbbf24"; 
                if (b.relationship === 'RESERVATION') ringColor = "#6366f1"; 
            ...
            {/* AIS TARGETS (Moving) */}
            {aisTargets.map((target, idx) => (
                <g key={`ais-${idx}`} transform={`translate(${50 + idx * 20}, 130)`}>
                    <circle r="4" className="fill-red-500/50 animate-ping" />
                    <circle r="2" className="fill-red-500" />
                    <text y="8" fontSize="3" className="fill-red-400" textAnchor="middle">{target.name}</text>
                </g>
            ))}
                ...
                >
                    <div className="flex justify-between items-start mb-4">
                        <div>
                            ...
                            <div className="text-xs font-mono text-zinc-500 mt-1">{selectedVessel.id}</div>
                        </div>
