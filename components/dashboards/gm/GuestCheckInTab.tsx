...
export const GuestCheckInTab: React.FC = () => {
  ...
  return (
    <div className="h-full flex flex-col space-y-4 bg-[#020617] p-4 animate-in fade-in duration-500">
      {/* Header */}
      <div className="flex items-center justify-between bg-zinc-900 p-4 rounded-xl border border-zinc-800 shrink-0">
        <div>
      ...
      <div className="flex-1 grid grid-cols-1 lg:grid-cols-2 gap-4 min-h-0">
                  ...
                  <div className="space-y-4 w-full max-w-xs">
                      {isScanning ? (
                          ...
                          <>
                            ...
                            <div>
                                <h4 className="font-bold text-zinc-300 text-sm">Identity Scan</h4>
                  ...
                  <div className="w-full animate-in zoom-in duration-300">
                      {checkInComplete ? (
                          <div className="space-y-4">
                              ...
                              <div className="text-sm font-bold text-zinc-200">ENTRY AUTHORIZED</div>
                              <button onClick={reset} className="text-xs text-zinc-500 underline">Next Guest</button>
                          </div>
                      ) : (
                          <div className="space-y-4 text-left">
                              <div className="bg-zinc-800 p-3 rounded text-xs space-y-2">
                                  <div><span className="text-zinc-500 block text-[9px] uppercase">Name</span><span className="font-bold text-white">{scannedData.fullName}</span></div>
                                  <div><span className="text-zinc-500 block text-[9px] uppercase">ID</span><span className="font-mono text-white">{scannedData.id}</span></div>
                              ...
                              <div className="flex gap-2">
                                  <button onClick={reset} className="flex-1 py-2 border border-zinc-700 rounded text-xs font-bold text-white hover:bg-zinc-800">CANCEL</button>
                                  <button onClick={handleCheckIn} className="flex-[2] py-2 bg-emerald-600 text-white rounded text-xs font-bold flex items-center justify-center gap-2 hover:bg-emerald-500">
          ...
          <div className="bg-zinc-900 rounded-xl border border-zinc-800 flex flex-col overflow-hidden">
              <div className="p-3 border-b border-zinc-800 bg-zinc-800/30 flex justify-between items-center shrink-0">
                  <span className="text-xs font-bold text-zinc-500 uppercase">Expected</span>
                  <button onClick={() => setShowScheduleForm(!showScheduleForm)} className="text-[10px] bg-zinc-800 border border-zinc-700 px-2 py-1 rounded hover:bg-zinc-700 text-white">+ Add</button>
              </div>
              <div className="flex-1 overflow-y-auto p-2 space-y-2 custom-scrollbar">
                  {showScheduleForm && (
                      <div className="p-2 bg-indigo-900/10 rounded border border-indigo-800/30 text-xs">
                          <input type="text" placeholder="Guest Name" className="w-full p-1 mb-1 rounded bg-zinc-800 border border-zinc-700 text-white focus:outline-none focus:border-indigo-500" />
                  ...
                  {expectedArrivals.map(item => (
                      <div key={item.id} onClick={() => !scannedData && setScannedData({ fullName: item.name, vesselName: 'Select Vessel' })} className="p-2 border border-zinc-800 rounded hover:bg-zinc-800 cursor-pointer">
                          <div className="flex justify-between">
                              <span className="text-xs font-bold text-white">{item.name}</span>
                              <span className="text-[9px] bg-blue-900/30 text-blue-400 px-1.5 py-0.5 rounded">{item.eta}</span>
                          </div>
