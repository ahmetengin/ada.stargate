...
export type SidebarTabId = 'ops' | 'fleet' | 'facility' | 'congress' | 'guest_checkin' | 'vhf' | 'observer' | 'presenter' | 'crm' | 'tech' | 'hr' | 'none';
...
const NodeItem = ({ id, label, state }: { id: string, label: string, state: string }) => (
    <div className="flex items-center justify-between px-3 py-2 group cursor-pointer hover:bg-white/5 rounded-lg transition-all">
  ...
  return (
    <div className="h-full w-full flex flex-col bg-[#020617] border-r border-white/5 text-slate-400 select-none overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar p-5 space-y-2">
          ...
          <NodeItem id="TRAF" label="Trafik Kontrol" state="idle" />
          <NodeItem id="PEDA" label="Akıllı Pedestallar" state="idle" />
          <NodeItem id="BOAT" label="Çekek / Lift" state="idle" />

          <SectionHeader title="ADA.FINANCE (CFO)" icon={CreditCard} color="text-emerald-400" />
          <NodeItem id="INVX" label="Fatura Otomasyonu" state="working" />
          <NodeItem id="YIEL" label="Dinamik Fiyatlama" state="idle" />

          <SectionHeader title="ADA.LEGAL (Counsel)" icon={Scale} color="text-indigo-400" />
          <NodeItem id="RAGX" label="Hukuki RAG" state="idle" />
          <NodeItem id="ISPS" label="Güvenlik & ISPS" state="idle" />

          <SectionHeader title="ADA.STARGATE (Beyin)" icon={Brain} color="text-purple-400" />
          <NodeItem id="ROUT" label="Orkestrasyon" state="working" />
          <NodeItem id="MESH" label="Dağıtık Ağ" state="idle" />
      ...
      <div className="p-4 border-t border-white/5 bg-[#050b14]">
          <div className="grid grid-cols-2 gap-2">
              <button onClick={() => onTabChange('observer')} className="flex flex-col items-center gap-1 p-3 rounded-xl bg-white/5 hover:bg-white/10 transition-all border border-white/5">
                <Activity size={16} className="text-slate-400" />
                <span className="text-[8px] font-black uppercase">Observer</span>
              </button>
              <button onClick={() => onTabChange('vhf')} className="flex flex-col items-center gap-1 p-3 rounded-xl bg-red-500/10 hover:bg-red-500/20 transition-all border border-red-500/20">
