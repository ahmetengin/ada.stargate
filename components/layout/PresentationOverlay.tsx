...
import { PresentationState, UserProfile, TenantConfig, AgentTraceLog } from '../../types';
import { generateSpeech } from '../../services/ai/geminiService';
...
export const PresentationOverlay: React.FC<PresentationOverlayProps> = ({ state, userProfile, onClose, onStartMeeting, onEndMeeting, onScribeInput, onStateChange, agentTraces, onArchive, activeTenantConfig }) => {
    ...
    useEffect(() => {
        if (state.isActive) {
            ...
            if (audioContextRef.current && audioContextRef.current.state !== 'closed') {
                audioContextRef.current.close().catch((e: any) => console.error(e));
            }
    ...
    const handleSendEmail = async () => {
        ...
        await executiveExpert.sendProposalEmail("theahmetengin@gmail.com", "WIM - 2025 Strategic Proposal & Minutes", state.analysisResults.proposal, () => {});
        setEmailStatus('SENT');
