...
def calculate_mooring_fee(loa: float, beam: float, days: int, season: str = "HIGH") -> str:
    ...
    """
    try:
        # 1. Constants (Hard Rules - Source of Truth)
...
def check_debt_status(vessel_id: str) -> str:
    """
    Mock worker for checking DB debt status.
    In prod, connects to Postgres.
    """
    # Simulation logic
    has_debt = True if "phisedelia" in vessel_id.lower() else False
    amount = 850.00 if has_debt else 0.00
    
    return json.dumps({
        "vessel_id": vessel_id,
        "status": "DEBT" if has_debt else "CLEAR",
        "amount_due": amount,
        "currency": "EUR",
        "block_departure": has_debt # Article H.2
    })
