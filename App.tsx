...
import { VoiceModal } from './components/modals/VoiceModal'; // IMPORTED
...
const App: React.FC = () => {
  ...
  const [isVoiceModalOpen, setIsVoiceModalOpen] = useState(false); // ADDED
  ...
  const handleStartPresentation = () => setPresentationState({ isActive: true, slide: 'intro', transcript: '', analysisResults: null });
  const handleClosePresentation = () => setPresentationState({ isActive: false, slide: 'intro', transcript: '', analysisResults: null });
  ...
  return (
    <div className={`w-screen h-screen font-sans flex flex-col transition-colors duration-300 ${theme === 'dark' ? 'dark' : ''}`}>
        ...
        <main className="flex-1 flex overflow-hidden">
          <div className="hidden lg:block w-[320px]">
              <Sidebar 
                  ...
                  onStartPresentation={handleStartPresentation} activeTenantId={activeTenantId} vhfLogs={vhfLogs}
              />
          </div>
      ...
      <DailyReportModal 
          ...
          userProfile={userProfile} weatherData={weatherData} activeTenantConfig={activeTenantConfig}
