...
# 🐍 Ada Stargate: Master Python Codebase

Bu dosya, Ada'nın "Bilişsel Varlık" (Cognitive Entity) sürümü için gerekli olan **Python Backend kodlarının referans kopyasıdır**.
Eğer otomatik oluşturma sırasında dosyalar eksik veya hatalı oluşursa, bu blokları kopyalayarak ilgili dosyalara yapıştırabilirsiniz.

---

### 1. Dosya: `backend/requirements.txt`
...
### 2. Dosya: `backend/ingest.py`
Hafıza Yükleyici (Learning Script).
...
if points:
    ...
    print(f"✅ SUCCESS: {len(points)} memories implanted.")
else:
...
### 3. Dosya: `backend/architecture_graph.py`
Ada'nın Beyni (LangGraph + MAKER + SEAL).
...
async def rag_retriever_node(state: AgentState):
    ...
    try:
        client = QdrantClient(url=QDRANT_URL)
        # Local Embedding kullanıyoruz (Docker içinde yüklü)
        embeddings = HuggingFaceEmbeddings(model_name="all-MiniLM-L6-v2")
